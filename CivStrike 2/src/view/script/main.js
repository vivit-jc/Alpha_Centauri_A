...
window.onload = function(){
    ...
    game.onload = function(){
        ...
        back.addEventListener('enterframe', function(){
    		if (help_text.active == false){set_help_text(0,-999,-1)}
    ...
    var Game_Player =  Class.create({
    	initialize:function(){
    		...
    		for (i = 0; i < MAP_WIDTH; i++) {
    			...
    			for (t = 0; t < MAP_HEIGHT; t++) {
    				this.map_sight[i][t] = [false,false,false,false,false,false,];
    			}
    		}
    	},
    	//viewにはtrue/falseを期待
    	set_sight:function(x,y,hex_id,view){
    		if (valid(x,y)){
	    		if (hex_id == -1){
	    			this.map_sight[x][y] = [true,true,true,true,true,true];
	    			return ;
	    		}
    	...
    	set_near_sight:function(x,y){
    		...
    		var ss_dir = [3,4,5];
    		for (i = 0; i < ss_dir.length; i++) {this.set_sight(x,y-1,ss_dir[i],true);}
    		var ss_dir = [0,1,2];
    		for (i = 0; i < ss_dir.length; i++) {this.set_sight(x,y+1,ss_dir[i],true);}
    		if (x % 2 == 1){
        		var ss_dir = [2,4,5];
        		for (i = 0; i < ss_dir.length; i++) {this.set_sight(x-1,y,ss_dir[i],true);}
        		var ss_dir = [0,3,4];
        		for (i = 0; i < ss_dir.length; i++) {this.set_sight(x+1,y,ss_dir[i],true);}
        		var ss_dir = [1,2,5];
        		for (i = 0; i < ss_dir.length; i++) {this.set_sight(x-1,y+1,ss_dir[i],true);}
        		var ss_dir = [0,1,3];
        		for (i = 0; i < ss_dir.length; i++) {this.set_sight(x+1,y+1,ss_dir[i],true);}
    		} else {
        		var ss_dir = [2,4,5];
    ...
    //●スタック表示Sprite
    var Sprite_Unit =  Class.create(Sprite, {
		...
		initialize:function(x,y,dir,col,id){
	   	...
	   	ontouchend:function(){
	   		...
	   		map_y = Math.floor((map_y - ((map_x % 2) * 55 - 54 + 16)) / 108);
	   		if (valid(map_x,map_y)){
		   		this.set_graphics(map_x,map_y,0);
		   		player_data.set_near_sight(map_x,map_y);
		   		refresh_worldmap();
	   		}
	   		//alert([this.base_x,this.base_y,map_x,map_y]);
	   	},
    ...
    var Sprite_HexTile =  Class.create(Sprite, {
    	...
    	initialize:function(pos_x,pos_y,i){
	   		 ...
	   		 this.image = game.assets['images/HexTile.png'];
    ...
    //●ヘックスの地形表示Sprite
    var Sprite_HexTriangle =  Class.create(Sprite, {
      	set_tile_graphics : function() {
	   		 var ind_x = Math.floor(this.pos_x / 3) ;
	   		 var ind_y = Math.floor((this.pos_y - Math.floor((this.pos_x % 6) / 3)) / 2) ;
	   		 var tile_id = 8;
	   		var hex_index = -1;
	   		 if (valid(ind_x,ind_y)){
		   		 var hex_data = world_hex[ind_x][ind_y];
		   		 var hex_index = this.pos_x % 3 + ((this.pos_y + Math.floor((this.pos_x % 6) / 3)) % 2 ) * 3;
		   		 var tile_id = HEX_TILE_PRITORITY[hex_data[0]][hex_data[hex_index + 1]] + 1;
				 this.visible = player_data.check_sight(ind_x,ind_y,hex_index);
	   		 }
	   		 this.frame = tile_id + (this.hex_ind % 2) * 10 ;
	   		 if (this.pos_y == 0 && ((this.pos_x % 6) / 3 >= 1)){
	   			this.frame = 0;
	   		 }
	   		 if (this.pos_y == HexTri_HEIGHT - 1 && ((this.pos_x % 6) / 3 < 1)){
		   		this.frame = 0;
	   		 }
        },
    	initialize:function(pos_x,pos_y,i){
   ...
   function start_game_phase(){
	   ...
	   create_new_unit(first_pos[0],first_pos[1]);
   ...
   function create_new_unit(x,y){
	   ...
	   var unit = new Game_Unit(x,y,-1);
	   unit.unit[0] = 1;
	   var ind = unit_sprites.length;
	   unit_sprites.push(new Sprite_Unit(x,y,-1,0,0));
...
function set_help_text(x,y,id){
	...
	if (help_text.id != id){
		help_text.id = id;
		if (id != -1){
			help_text.text = DeckContents[id][2];
		} else {
			help_text.text = '';
		}
		help_text.x = x;
...
function change_view_deck(){
    ...
    for (i = 0; i < 10 * 6; i++) {
    	...
    	if (index != undefined){
    		...
    		deck_contents_sprites[i].frame = data[3];
...
function set_inventory_contents(i,data){
	...
	if (data != -1){
		inventory_contents_sprites[i].frame = data[3];
